...
def run_import_postprocess_job_deferred(
    *,
    ...
    try:
        worker = threading.Thread(target=_run, name="import-postprocess-deferred", daemon=True)
        worker.start()
        return {"deferred": True, "queued": True, "dedupe_key": dedupe_key}
    except Exception as error:
        with _POSTPROCESS_DEFER_LOCK:
            _POSTPROCESS_IN_FLIGHT.discard(dedupe_key)
        _persist_postprocess_state(
        ...
        log_operational_event(
            "import_postprocess_deferred_start_failed",
            status="failed",
            tenant_id=tenant_id,
            detail="Falling back to synchronous postprocess because deferred runner failed to start.",
            context={"error": str(error), "row_count": len(payload_rows)},
        )
        _persist_postprocess_state(
            tenant_id=tenant_id,
            source_upload_id=source_upload_id,
            state="running_sync_fallback",
            source_import_job_id=source_import_job_id,
            row_count=len(payload_rows),
            dedupe_key=dedupe_key,
        )
        try:
            ...
            _persist_postprocess_state(
                tenant_id=tenant_id,
                source_upload_id=source_upload_id,
                state="completed_sync_fallback",
                source_import_job_id=source_import_job_id,
                row_count=len(payload_rows),
                dedupe_key=dedupe_key,
            )
            return {"deferred": False, "queued": False, "fallback_sync": True}
            ...
            log_operational_event(
                ...
                status="failed",
                tenant_id=tenant_id,
                detail="Deferred import postprocess start fallback failed.",
                context={"error": str(fallback_error), "row_count": len(payload_rows)},
            )
            log_operational_event(
                "import_postprocess_needs_manual_recovery",
                status="failed",
            ...
            return {"deferred": False, "queued": False, "fallback_sync": True, "failed": True}
